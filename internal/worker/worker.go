...
func (w *Worker) EnsureRepo(ctx context.Context, cloneURL, repo string) (string, error) {
	...
	if _, err := os.Stat(filepath.Join(repoDir, ".git")); os.IsNotExist(err) {
		...
		exec.CommandContext(ctx, "git", "-C", repoDir, "config", "user.email", "claude-afk@bot").Run()
		exec.CommandContext(ctx, "git", "-C", repoDir, "config", "user.name", "claude-afk").Run()
	} else {
...
func (w *Worker) ProcessIssue(ctx context.Context, repo string, issue *github.Issue) {
	...
	if err != nil {
		w.fail(key, logger, fmt.Sprintf("ensure repo: %v", err))
		return
	}
	...
	if strategy == "pr" {
		...
		cmd.Stderr = os.Stderr
		if err := cmd.Run(); err != nil {
			w.fail(key, logger, fmt.Sprintf("checkout branch: %v", err))
			return
		}
	}
	...
	if strings.Contains(result, "ISSUE_FAILED") {
		w.fail(key, logger, fmt.Sprintf("claude reported failure: %s", result))
		return
	}
	...
	if strategy == "pr" {
		...
		cmd.Stderr = os.Stderr
		if err := cmd.Run(); err != nil {
			w.fail(key, logger, fmt.Sprintf("git push branch: %v", err))
			return
		}
		...
		if len(parts) != 2 {
			w.fail(key, logger, fmt.Sprintf("invalid repo format: %s", repo))
			return
		}
		...
		if err != nil {
			w.fail(key, logger, fmt.Sprintf("create PR: %v", err))
			return
		}
		...
		cmd.Stderr = os.Stderr
		if err := cmd.Run(); err != nil {
			w.fail(key, logger, fmt.Sprintf("git push: %v", err))
			return
		}
		w.State.Set(key, state.IssueState{
			Status:      state.StatusCompleted,
...
func (w *Worker) fail(key string, logger *slog.Logger, reason string) {
	...
	w.State.Save()
	w.Notifier.Send(fmt.Sprintf("claude-afk failed %s: %s", key, reason))
}
